...
class MDFEngine(BaseEngine):
    ...
    def _check_classification_leakage(
        ...
        if len(X) <= 500:
            # 小数据集：置换检验（100次）
            p_value = self._permutation_test_mi(X, y, mi, config.random_state, n_perm=100)
        else:
            # 大数据集：卡方检验近似
            p_value = self._chi2_test_mi(X, y)

        # 风险判断 - 两级检测
        # high: 相关系数 > 0.98（几乎完全相同，直接泄露）
        # medium: 相关系数 > 0.85（异常强关联，可能泄露）
        if mi > config.mi_high and p_value < config.p_value_threshold:
            ...
            if abs(correlation) > 0.98:
                ...
                return LeakageItem(
                    ...
                    fix_hint="该特征与标签几乎完全相同，极可能是直接泄露",
                )
                ...
                return LeakageItem(
                    ...
                    taxonomy_ref="Kapoor & Narayanan 2023, Type 5",
                    risk_level="medium",
                    risk_score=min(0.75, 0.5 + abs(correlation) * 0.3),
                    affected_count=len(X),
                    affected_ratio=1.0,
                    detail=f"特征 '{feature_col}' 与标签存在异常强关联 "
                    f"(MI={mi:.4f}, corr={correlation:.4f}, p={p_value:.4e})",
                    fix_hint="该特征与标签存在异常强关联，请确认预测时是否可获得",
                )

        return None

    def _permutation_test_mi(
        self, X: np.ndarray, y: np.ndarray, observed_mi: float, random_state: int, n_perm: int = 100
    ) -> float:
        """
        置换检验计算互信息的 p-value
